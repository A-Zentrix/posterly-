    ...
    try:
        ...
        try:
            ...
            if not poster_path or not Path(poster_path).exists():
                raise Exception("Poster file was not created.")
            if Path(poster_path).stat().st_size < 1000:
                raise Exception("Poster file is too small to be valid.")
        ...
        try:
            ...
            if not final_path:
                raise Exception("Watermarking failed, output path not returned.")
